...
// Define thresholds for correct form for each exercise
const EXERCISE_THRESHOLDS = {
  ...
  "db-incline-fly": {
    shoulder: { min: 45, max: 180 }, // This range covers both start and end of the movement
    elbow: { min: 150, max: 170 }, // Elbow should maintain a slight, fixed bend
  },
};
...
function MonitorInner() {
  ...
  // State for shoulder angles and their positions
  ...
  // State for elbow angles and their positions
  ...
  // States to track correctness of form
...
useEffect(() => {
    ...
    (async () => {
      ...
      pose.setOptions({
        selfieMode: true,
        modelComplexity: 1, // You can adjust this if needed
  ...
  useEffect(() => {
    ...
    // Helper function to draw an angle arc on the canvas
      ...
      const onResults = (results: any) => {
        ...
        const updateElbowAngles = (landmarks: any) => {
            ...
            if (leftShoulder && leftElbow && leftWrist && leftElbow.visibility > 0.5) {
                ...
                drawAngleArc(ctx, leftElbow, leftShoulder, leftWrist, lCorrect ? '#00FF00' : '#FF0000');
            }
            if (rightShoulder && rightElbow && rightWrist && rightElbow.visibility > 0.5) {
                ...
                drawAngleArc(ctx, rightElbow, rightShoulder, rightWrist, rCorrect ? '#00FF00' : '#FF0000');
            }
        ...
        const updateShoulderAngles = (landmarks: any) => {
            if (!landmarks) return;
            ...
            if (leftHip && leftShoulder && leftElbow && leftShoulder.visibility > 0.5) {
                ...
                drawAngleArc(ctx, leftShoulder, leftHip, leftElbow, lCorrect ? '#00FF00' : '#FF0000');
            }
            if (rightHip && rightShoulder && rightElbow && rightShoulder.visibility > 0.5) {
                ...
                drawAngleArc(ctx, rightShoulder, rightHip, rightElbow, rCorrect ? '#00FF00' : '#FF0000');
            }
        ...
        if (results.poseLandmarks) {
          updateShoulderAngles(results.poseLandmarks);
          updateElbowAngles(results.poseLandmarks);
  ...
  }, [isModalOpen, modalTarget, exerciseId]);
  

  useEffect(() => {
    const id = setInterval(() => {
      setDuration(d => d + 1);
      setReps(r => ((duration + 1) % 5 === 0 ? r + 1 : r));
      setProgressWidth(w => Math.min(100, Math.round(((duration + 1) / 60) * 100)));
    }, 1000);
      ...
      <div className="mx-auto w-full max-w-screen-2xl rounded-3xl bg-white p-5 sm:p-8 shadow-xl ring-1 ring-black/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="grid gap-4">
            ...
            {phaseList.length > 0 && (
              <div>
                <p className="text-sm font-medium text-gray-500">Selected Phases:</p>
                <p className="font-bold text-gray-800 break-words">{phaseList.join(", ")}</p>
              </div>
            )}
            <div>
              ...
              {(!modalTarget || !((modalTarget === "front" ? streamFrontRef.current : streamSideRef.current))) ? (
                ...
                <div className="w-full h-full flex-grow relative grid place-items-center overflow-hidden">
                  ...
                  {leftShoulderPos.visible && (
                    ...
                    >
                      <p>L-Shoulder: {Math.round(leftShoulderAngle)}°</p>
                    </div>
                  )}
                  ...
                  {rightShoulderPos.visible && (
                    ...
                    >
                      <p>R-Shoulder: {Math.round(rightShoulderAngle)}°</p>
                    </div>
                  )}
                  ...
                  {leftElbowPos.visible && (
                    ...
                    >
                      <p>L-Elbow: {Math.round(leftElbowAngle)}°</p>
                    </div>
                  )}
                  ...
                  {rightElbowPos.visible && (
                    ...
                    >
                      <p>R-Elbow: {Math.round(rightElbowAngle)}°</p>
                    </div>
                  )}
                </div>
              )}
