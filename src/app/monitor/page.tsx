  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const onResults = (results: any) => {
        ...
        const detectPhase = (shoulderAngle: number) => {
            if (!EXERCISE_PHASES[exerciseId as keyof typeof EXERCISE_PHASES]) return;
            const phases = EXERCISE_PHASES[exerciseId as keyof typeof EXERCISE_PHASES];
            const current = phases.find(p => shoulderAngle >= p.angle.min && shoulderAngle <= p.angle.max);
            setCurrentPhase(current ? current.name : "Unknown Phase");
        }
        ...
        if (results.poseLandmarks) {
          ...
          const currentShoulderAngle = leftShoulderAngle || rightShoulderAngle;
          if (currentShoulderAngle) {
              detectPhase(currentShoulderAngle);
          }
